 ...
 *	File: vtkJointContourNetForShapes.cxx
...
#include "vtkJointContourNetForShapes.h"
...
vtkStandardNewMacro(vtkJointContourNetForShapes);
...
void vtkJointContourNetForShapes::AddField(char *nm, double wd)
{
  Field *f = new Field();
...
// Caller's responsibility to ensure field is not duplicated.
void vtkJointContourNetForShapes::AddField(char *nm, double wd, double base)
{
  Field *f = new Field();
  f->name = new char[strlen(nm) + 1];
  strcpy(f->name, nm);
...
char * vtkJointContourNetForShapes::GetField(int n)
{
...
int vtkJointContourNetForShapes::GetNumberOfFields()
{
...
// Clear all fields.
void vtkJointContourNetForShapes::ClearFields()
{
  this->Fields.clear();
}
...
vtkJointContourNetForShapes::vtkJointContourNetForShapes()
{
...
void vtkJointContourNetForShapes::PrintSelf(ostream& os, vtkIndent indent)
{
...
vtkIdType vtkJointContourNetForShapes::Find(vtkIdType x)
{
...
int vtkJointContourNetForShapes::FillInputPortInformation(int, vtkInformation* info)
{
...
int vtkJointContourNetForShapes::FillOutputPortInformation(int ignored, vtkInformation* info)
{
  // Port 0 = JCN graph
...
vtkDataObject* vtkJointContourNetForShapes::GetOutput(int index)
{
  vtkDataObject *obj;
  if (index == 0)
    {
...
vtkUndirectedGraph *vtkJointContourNetForShapes::GetTopologyGraph()
{
...
int vtkJointContourNetForShapes::RequestDataObject(
...
double vtkJointContourNetForShapes::SlabCoordinate(int f, double v)
{
...
// This method makes extensive use of vtkPolytopeGeometryForShapes, and
...
int vtkJointContourNetForShapes::RequestData(vtkInformation*,
                                 vtkInformationVector** inputVector,
                                 vtkInformationVector* outputVector)
{
  ...
  // the bounds array.
	//cout<<"\n\nMaxDomainDimension :"<<MaxDomainDim;
	//cout<<"\n\nMaxRangeDimension :"<<MaxRangeDim;
	//cout<<"\n\nDimension : :"<<this->M<<"\n\n";
  for (int d = 0; d < (this->M + 1); d++)
     {
  ...
  vtkSmartPointer<vtkPolytopeGeometryForShapes> geometry 
      = vtkSmartPointer<vtkPolytopeGeometryForShapes>::New();
 ...
 // Points for the output grid.  Output points will be a superset of the
  ...
  // Array dimensions use constants from vtkPolytopeGeometryForShapes.
  // ------------------------------------------------------
  ...
  // polytopes returned by vtkPolytopeGeometryForShapes::Slice.
  ...
  // global spatial bounds.
	//cout<<"\n\nNumber of fields : "<<this->N<<"\n";
  geometry->Initialize(this->M + 1, this->N, dataBounds);
  // ------------------------------------------------------
  // MAIN LOOP:
  //
  ...
  for (vtkIdType c = 0; c < numCells; c++)
    ...
    if (this->UseCellTopologyArray)
      ...
      {
      input->GetCellPoints(c, pids);//Stores the point ids of the three vertices in pids
      points->GetTuple(pids->GetId(0), point);//Store the vertex of index pids->GetId(0) in point
      }
...
//	cout<<"\npid : "<<pID<<"\n\n";
	...
	//cout<<"\nv : "<<v<<" i :"<<i<<" "<<this->Fields[i]->pointScalars->GetComponent(pID, 0)<<"\n\n";
	...
	//cout<<"\nnumber of vertex ids in a face  End: "<<polyNr<<"\n\n";
...
//cout<<"\nNumber of fragments : "<<nrFragments;
...
//		cout<<"\nFragment : "<<i<<" Slab number : "<<floor((fv + EPSILON - minF) / sw);
      }
    }
 ...
 // vtkIdType nrFacets = facets->GetNumberOfTuples();
    vtkIdType nrFacets = geometry->edgeTriangleMap.size();
//  vtkIdTypeArray *facetsId = geometry->GetCenterFacetsId();
  ...
  /*for (vtkIdType i = 0; i < nrFacets; i++)
    ...
    for (map<vtkIdType, vector<int> >::iterator itr = geometry->edgeTriangleMap.begin(); itr != geometry->edgeTriangleMap.end(); ++itr)
    {
	vector<int>& triangles= itr->second;
	for(int x = 0; x < triangles.size(); x++){
		for(int y = x+1; y < triangles.size(); y++){
			u = triangles[x];
			v = triangles[y];
			// u and v are polytopes that share facet i.
			// Check whether the slab number of each component
			...
			{
				// Note: slab numbers are short integers: no epsilon involved.
				equal = this->Fields[j]->slabNumbers->GetComponent(u, 0) == this->Fields[j]->slabNumbers->GetComponent(v, 0);                
			}
			// If all components are equal, merge the classes.
			if (equal)
			{
  ...
  for (vtkIdType i = 0; i < nrFragments; i++)
	...
	//cout<<"\nNumber of slabs :"<<nrSlabs<<"\n";
  //  cout<<"\n\nroot setting done find done \n\n";
...
//    cout<<"\nNumber of facets : "<<nrFacets<<"\n\n";
 ...
 //   cout<<"\nTopograph constructed :\n";
  ...
  /*if (this->IdentifyBoundarySlabs)
    {
//		cout<<"\nInside identify boundary slabs\n\n";
